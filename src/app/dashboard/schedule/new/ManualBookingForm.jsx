...
export default function ManualBookingForm({ clients: initialClients, services, sitterName, initialDate }) {
  ...
  async function run(work, reset = true) {
  ...
  if (review) return <section className="min-w-0 max-w-full space-y-4 [overflow-wrap:anywhere]" aria-label="Review booking">
    ...
    {summary && <FormFeedback>{summary}</FormFeedback>}
    <div className="flex flex-wrap gap-3"><Button disabled={busy} onClick={save}>{busy ? "Saving…" : `Save Booking · ${money(review.summary.clientTotalCents)}`}</Button><Button disabled={busy} variant="secondary" onClick={() => { setReview(null); }}>Edit details</Button></div>
  </section>;

  return <form onSubmit={submit} onChange={(event) => event.target.id !== "service" && recover(event.target.dataset.errorField || event.target.id)} className="min-w-0 max-w-full space-y-5 [overflow-wrap:anywhere]">
    <fieldset disabled={busy} className="min-w-0 space-y-5">
      <Card className="space-y-4 p-4">
        <h2 className="text-lg font-bold">Client</h2>
        <div className="flex gap-2"><Button variant={!newClient ? "primary" : "secondary"} onClick={() => { if (newClient) recover("client-mode"); setNewClient(false); }}>Existing client</Button><Button variant={newClient ? "primary" : "secondary"} onClick={() => { if (!newClient) recover("client-mode"); setNewClient(true); }}>New client</Button></div>
        {newClient ? <>
          ...
          <div className="flex items-end gap-2"><BookingField id="client-search" label="Find client" className="min-w-0 flex-1" placeholder="Name, email, or phone" value={search} onChange={(event) => setSearch(event.target.value)} /><Button variant="secondary" onClick={() => run(async () => {
            ...
            if (!result.ok) { showFailure(result); return; }
            setClients(result.clients); setClientId(""); setPetIds([]); recover("client-select");
      ...
      <Card className="space-y-4 p-4">
        ...
        <BookingField id="service" error={fieldErrors["serviceCode"]} label="Service" as="select" required value={serviceCode} onChange={(event) => {
          ...
          recover(next?.category === "OVERNIGHT" ? "service-overnight" : "service");
        ...
        {extraServices.length > 0 && <details open={extrasOpen} onToggle={(event) => setExtrasOpen(event.currentTarget.open)}><summary className="cursor-pointer py-2 font-semibold">Extras (optional)</summary><p className="my-2 text-sm">Enter the total quantity for the whole booking, including extra-pet charges when applicable.</p><div className="space-y-3">{extraServices.map((extra) => <BookingField key={extra.code} id={`extra-${extra.code}`} error={fieldErrors[`extra:${extra.code}`] || fieldErrors.extras} label={`${extra.name} · ${money(extra.basePriceCents)} each`} type="number" min="0" max="366" step="1" inputMode="numeric" value={extras[extra.code] || ""} onChange={(event) => setExtras({ ...extras, [extra.code]: event.target.value })} />)}</div></details>}
      </Card>
