...
export default function ManualBookingForm({ clients: initialClients, services, sitterName, initialDate }) {
  ...
  async function submit(event) {
    ...
    await run(async () => {
      if (!newClient && !clientId) { setError("Select an existing client or choose New client."); return; }
  ...
  async function save() {
    await run(async () => {
      ...
      if (!result.ok) {
        ...
        if (["PRICE_CHANGED", "REVIEW_REQUIRED", "VISIT_ALREADY_STARTED", "SCHEDULE_CONFLICT"].includes(result.code)) setReview(null);
        return;
      }
  ...
  if (review) return <section className="space-y-4" aria-label="Review booking">
  ...
  return <form onSubmit={submit} className="space-y-5">
    <fieldset disabled={busy} className="min-w-0 space-y-5">
      <Card className="space-y-4 p-4">
        ...
        <div className="flex gap-2"><Button variant={!newClient ? "primary" : "secondary"} onClick={() => setNewClient(false)}>Existing client</Button><Button variant={newClient ? "primary" : "secondary"} onClick={() => setNewClient(true)}>New client</Button></div>
        {newClient ? <>
          <FormField id="client-name" label="Client name" required maxLength={200} autoComplete="name" value={client.name} onChange={(event) => setClient({ ...client, name: event.target.value })} />
          <FormField id="client-email" label="Email (optional)" type="email" maxLength={254} autoComplete="email" value={client.email} onChange={(event) => setClient({ ...client, email: event.target.value })} />
          <FormField id="client-phone" label="Phone (optional)" type="tel" maxLength={50} autoComplete="tel" value={client.phone} onChange={(event) => setClient({ ...client, phone: event.target.value })} />
          <details><summary className="cursor-pointer py-2 font-semibold">Address (optional)</summary><div className="mt-3 space-y-3">{[["addressLine1", "Street address"], ["addressLine2", "Apartment / unit"], ["city", "City"], ["state", "State"], ["postalCode", "ZIP code"]].map(([key, label]) => <FormField key={key} id={`client-${key}`} label={label} maxLength={200} value={client[key]} onChange={(event) => setClient({ ...client, [key]: event.target.value })} />)}</div></details>
        </> : <>
          <div className="flex items-end gap-2"><FormField id="client-search" label="Find client" className="min-w-0 flex-1" placeholder="Name, email, or phone" value={search} onChange={(event) => setSearch(event.target.value)} /><Button variant="secondary" onClick={() => run(async () => {
            const result = await searchScheduleClients(search);
            if (!result.ok) { setError(result.error); return; }
            setClients(result.clients); setClientId(""); setPetIds([]);
          })}>Search</Button></div>
          <FormField id="client-select" label="Client" as="select" required value={clientId} onChange={(event) => { setClientId(event.target.value); setPetIds([]); }}>
          ...
          {!!selectedClient?.pets.length && <fieldset><legend className="text-sm font-semibold">Pets (optional)</legend>{selectedClient.pets.map((pet) => <label key={pet.id} className="flex min-h-11 items-center gap-3"><input type="checkbox" checked={petIds.includes(pet.id)} onChange={(event) => setPetIds(event.target.checked ? [...petIds, pet.id] : petIds.filter((id) => id !== pet.id))} />{pet.name} ({pet.species})</label>)}</fieldset>}
        </>}
      </Card>
      <Card className="space-y-4 p-4">
        <h2 className="text-lg font-bold">Service and dates</h2>
        <FormField id="service" label="Service" as="select" required value={serviceCode} onChange={(event) => {
        ...
        }}>{primaryServices.map((item) => <option key={item.code} value={item.code}>{item.name} · {money(item.basePriceCents)}</option>)}</FormField>
        {!primaryServices.length && <Notice>No active services are available. Contact the operator.</Notice>}
        {overnight ? <div className="grid gap-3 sm:grid-cols-2">{[["arrivalDate", "Arrival date", "date"], ["departureDate", "Departure date", "date"], ["arrivalTime", "Arrival time", "time"], ["departureTime", "Departure time", "time"]].map(([key, label, type]) => <FormField key={key} id={key} label={label} type={type} required value={stay[key]} onChange={(event) => setStay({ ...stay, [key]: event.target.value })} />)}</div> : <>
          ...
          {visits.map((visit, index) => <fieldset key={index} className="min-w-0 space-y-3 border-t border-[var(--task-border)] pt-3"><legend className="font-semibold">Visit {index + 1}</legend>
            <FormField id={`date-${index}`} label="Date" type="date" required value={visit.date} onChange={(event) => setVisits(visits.map((item, i) => i === index ? { ...item, date: event.target.value } : item))} />
            <div className="grid grid-cols-2 gap-3"><FormField id={`time-${index}`} label="Start time" type="time" min="07:00" max="22:00" required value={visit.startTime} onChange={(event) => setVisits(visits.map((item, i) => i === index ? { ...item, startTime: event.target.value, endTime: endTime(event.target.value, service?.durationMinutes) } : item))} /><FormField id={`end-${index}`} label="End time" type="time" value={visit.endTime} readOnly /></div>
            {visits.length > 1 && <Button variant="quiet" onClick={() => setVisits(visits.filter((_, i) => i !== index))}>Remove visit {index + 1}</Button>}
          </fieldset>)}
        ...
        {extraServices.length > 0 && <details><summary className="cursor-pointer py-2 font-semibold">Extras (optional)</summary><p className="my-2 text-sm">Enter the total quantity for the whole booking, including extra-pet charges when applicable.</p><div className="space-y-3">{extraServices.map((extra) => <FormField key={extra.code} id={`extra-${extra.code}`} label={`${extra.name} · ${money(extra.basePriceCents)} each`} type="number" min="0" max="366" step="1" inputMode="numeric" value={extras[extra.code] || ""} onChange={(event) => setExtras({ ...extras, [extra.code]: event.target.value })} />)}</div></details>}
      </Card>
      <FormField id="notes" label="Care notes (optional)" as="textarea" maxLength={1000} value={notes} onChange={(event) => setNotes(event.target.value)} hint="Care instructions are saved with this booking." />
