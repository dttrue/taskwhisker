...
for (const waived of [false, true]) test(`legacy cancellation unchanged with waived=${waived}`, async () => {
  ...
  let write;
  const tx = { booking: { findUnique: async () => booking, updateMany: async ({ data }) => { write = data; return { count: 1 }; } }, visit: { updateMany: async ({ where }) => { assert.deepEqual(where.status.in, ["PENDING", "CONFIRMED"]); } }, bookingHistory: { create: async () => {} }, conversation: { upsert: async () => ({ id: "c" }) }, message: { create: async () => {} } };
